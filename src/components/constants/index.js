export { default as Navbar } from './Navbar';
export { default as NavbarMobile } from './NavbarMobile';
export { default as PackNav } from './PackNav';
export { default as PackItem } from './PackItem';
export { default as Carousel } from './Carousel';
export { default as CarouselItem } from './CarouselItem';
export { default as Footer } from './Footer';
export { default as PackNavList } from './PackNavList';
export { default as PackNavMobile } from './PackNavMobile';
export { default as PackNavListMobile } from './PackNavListMobile';

// This index.js file allows me to call these components on a single line in Home.jsx
